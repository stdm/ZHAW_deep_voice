...
class SegmentBatchIterator(BatchIterator):
    ...
    def __iter__(self):
        ...
        for i in range((self.n_samples + bs - 1) // bs):
            Xb = np.zeros((bs, 1, self.config.getint('luvo', 'spectogram_height'),
            ...
            for j in range(0, bs):
                ...
                spectrogramm = extract_spectrogram(self.X[speaker_idx, 0], self.config.getint('luvo', 'seg_size'),
                                                   self.config.getint('luvo', 'spectogram_height'))
                seg_idx = randint(0, spectrogramm.shape[1] - self.config.getint('luvo', 'seg_size'))
                Xb[j, 0] = spectrogramm[:, seg_idx:seg_idx + self.config.getint('luvo', 'seg_size')]
            yield Xb, yb
